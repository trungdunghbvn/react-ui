  ...
  return (name, filename) => {
    ...
    const polarisComponentName =
      nestedComponentMatch && nestedComponentMatch.length > 1
        ? `${autoketingClassName(nestedComponentMatch[1])}-${componentName}`
        : autoketingClassName(componentName);
...
function autoketingClassName(className) {
  return `Autoketing-${className}`;
}
